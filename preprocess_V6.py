...
DEFAULT_OUTPUT_DIR = 'processed_data_v6'

# Processing parameters
MAX_SEQ_LENGTH = 200  # For storage only; dynamic batching will re-pad
RANDOM_SEED = 42
...
def filter_trajectories(trajectories, max_length=195, max_distance_ratio=3.0):
    """
    ...
    Args:
        ...
        max_length: Maximum allowed sequence length (default: 200)
        max_distance_ratio: Maximum actual/ideal distance ratio (default: 3.0)
    ...
    print(f"  3. Ideal distance < 10px (too short/start == end)")
    ...
    filtered_both = []
    filtered_zero_distance = []

    for traj in trajectories:
        extracted_length = len(traj['x'])
        # Check for zero or near-zero ideal_distance first (prevent division by zero)
        if traj["ideal_distance"] < 20:  # Less than 10 pixels is essentially zero movement
            filtered_zero_distance.append({
    ...
    total_filtered = len(filtered_too_long) + len(filtered_too_indirect) + len(filtered_both) + len(filtered_zero_distance)
    ...
    if filtered_zero_distance:
        print(f"\n[FILTERED - ZERO DISTANCE] ({len(filtered_zero_distance)} files)")
        print(f"  (ideal_distance < 10px - start == end or too short)")
        for f in sorted(filtered_zero_distance, key=lambda x: x['ideal_distance']):
            print(f"  - {f['filename']}")
            print(f"      ideal={f['ideal_distance']:.1f}px, actual={f['actual_distance']:.1f}px, length={f['extracted_length']}")
...
# ============================================================================
# STEP 3.0: CLIP OUTLIERS (CRITICAL FOR TRAINING)
# ============================================================================
...
def clip_outliers(trajectories, percentile_low=1, percentile_high=99):
    ...
    Without clipping, extreme values (e.g., acceleration of 10,000,000 px/sÂ²)
    ...
    """
    print(f"\n{'='*70}")
    print("STEP 3.0: Clipping Outliers (Percentile-Based)")
    print(f"{'='*70}")
    ...
    print(f"  accel:   [{accel_low:12.2f}, {accel_high:12.2f}] px/sÂ²")
...
def normalize_goal_conditions(trajectories, norm_params):
    """
    Compute 3D goal conditioning: [distance, cos(goal_angle), sin(goal_angle)]

    This addresses the directional ambiguity in V6 Stage 2 training.
    Instead of 1D distance (which creates 8-fold ambiguity for N/NE/E/SE/S/SW/W/NW),
    we use 3D conditioning that uniquely specifies both distance and direction.

    Returns:
        trajectories: Updated with 'condition' key (3D array)
        norm_params: Updated with goal normalization parameters
    """
    print(f"\n{'='*70}")
    print("STEP 4: Computing 3D Goal Conditioning")
    print(f"{'='*70}")
    print("Condition format: [distance_norm, cos(goal_angle), sin(goal_angle)]")
    ...
    for traj in trajectories:
        ...
        # Compute goal vector (start → end)
        dx_goal = x[-1] - x[0]
        dy_goal = y[-1] - y[0]

        # Goal distance (Euclidean distance from start to end)
        # Note: This is ideal_distance already computed
        distance = traj['ideal_distance']

        # Goal angle (direction from start to end)
        angle = np.arctan2(dy_goal, dx_goal)  # Range: [-π, π]

        actual_distances.append(traj['actual_distance'])
        ideal_distances.append(distance)
        goal_angles.append(angle)

        # Store raw values for later normalization
        traj['goal_distance_raw'] = distance
        traj['goal_angle_raw'] = angle
        traj['goal_dx_raw'] = dx_goal
        traj['goal_dy_raw'] = dy_goal
    ...
    # Using ideal_distance (straight-line) as the primary conditioning variable
    d_min, d_max = ideal_distances.min(), ideal_distances.max()
    distances_norm = 2 * ((ideal_distances - d_min) / (d_max - d_min)) - 1

    # Encode angle as [cos(angle), sin(angle)]
    # This is already normalized to [-1, 1] and avoids discontinuity at ±180°
    cos_angles = np.cos(goal_angles)
    sin_angles = np.sin(goal_angles)

    # Build 3D conditions
    for i, traj in enumerate(trajectories):
        traj['condition'] = np.array([
            distances_norm[i],  # Normalized distance
            cos_angles[i],      # X direction component
            sin_angles[i]       # Y direction component
        ], dtype=np.float32)

    # Store normalization parameters
    norm_params['goal_dist_min'] = float(d_min)
    norm_params['goal_dist_max'] = float(d_max)
    norm_params['actual_dist_min'] = float(actual_distances.min())
    norm_params['actual_dist_max'] = float(actual_distances.max())

    # Statistics
    print(f"\nGoal distance (ideal): [{d_min:.1f}, {d_max:.1f}] px")
    print(f"Actual distance (path): [{actual_distances.min():.1f}, {actual_distances.max():.1f}] px")
    print(f"Curvature ratio (actual/ideal): {actual_distances.mean() / ideal_distances.mean():.2f}x")
    print(f"\nNormalized distance: [{distances_norm.min():.4f}, {distances_norm.max():.4f}]")
    print(f"Goal angles: [{np.degrees(goal_angles.min()):.1f}°, {np.degrees(goal_angles.max()):.1f}°]")
    print(f"Cos(angle): [{cos_angles.min():.4f}, {cos_angles.max():.4f}]")
    print(f"Sin(angle): [{sin_angles.min():.4f}, {sin_angles.max():.4f}]")

    # Analyze direction distribution
    angle_bins = np.linspace(-np.pi, np.pi, 9)  # 8 directions
    hist, _ = np.histogram(goal_angles, bins=angle_bins)
    print(f"\nDirection distribution (8 bins):")
    directions = ['E', 'NE', 'N', 'NW', 'W', 'SW', 'S', 'SE']
    for i, (direction, count) in enumerate(zip(directions, hist)):
        pct = (count / len(goal_angles)) * 100
        bar = '█' * int(pct / 2)
        print(f"  {direction:3s}: {count:4d} ({pct:5.1f}%) {bar}")
...
def pad_sequences(trajectories, max_length=200):
    """
    ...
    for i, traj in enumerate(trajectories):
        ...
        padded_data.append({
            ...
            'condition': traj['condition'],  # 3D: [distance_norm, cos(angle), sin(angle)]
            'actual_distance': traj['actual_distance'],
            'ideal_distance': traj['ideal_distance'],
            'goal_angle': traj['goal_angle_raw'],  # Add for reference
            'filename': traj['filename']
        })
...
def create_splits(padded_data, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2, seed=42):
    ...
    C = np.stack([p['condition'] for p in padded_data])  # Shape: (n_samples, 3)
    ...
    assert C.shape[1] == 3, f"Expected condition_dim=3, got {C.shape[1]}"
...
def main():
    ...
    parser.add_argument(
        ...
        help='Maximum sequence length for storage/padding (default: 200)'
    )
    parser.add_argument(
        '--max_filter_length',
        type=int,
        default=195,
        help='Maximum trajectory length before filtering (default: 195)'
    )
    parser.add_argument(
        '--max_distance_ratio',
        type=float,
        default=3.0,
        help='Maximum actual/ideal distance ratio before filtering (default: 3.0)'
    )
    parser.add_argument(
        '--no_filter',
        action='store_true',
    ...
    trajectories, norm_params = normalize_goal_conditions(trajectories, norm_params)
